...
class _MyPageState extends State<MyPage> {
  ...
  BackupNotificationSettings _notificationSettings =
      BackupNotificationSettings.defaults();
  RestoreSourceGrant? _restoreSourceGrant;
  BusinessRegistrySnapshotInfo? _registrySnapshot;
  ...
  BackupNotificationPermissionPort get _permissionPort {
    ...
    if (notificationPort is BackupNotificationPermissionPort) {
  ...
  ScheduledBackupReminderPort get _schedulePort {
    ...
    if (explicit != null) return explicit;
    final Object notificationPort = widget.notificationPort;
    if (notificationPort is ScheduledBackupReminderPort) {
      return notificationPort;
    }
    return const NoopScheduledBackupReminderPort();
  }

  bool get _registryDistributionConfigured =>
    ...
    return Scaffold(
      ...
      body: BackupMigrationCenter(
        ...
        registryUpdateSection: BusinessRegistryUpdateCard(
          ...
          distributionConfigured: _registryDistributionConfigured,
          statusMessage: _registryStatusMessage,
        ...
        onReadableImportReview: () async {
          ...
          await _showSafeSource(
            context,
            () => widget.actionService.prepareReadableImportSource(
              currentDb: db,
            ),
            currentDb: db,
          );
  ...
  Future<BusinessRegistryRepository> _businessRegistryRepository() async {
    final explicit = widget.registryRepository;
    if (explicit != null) return explicit;
    return BusinessRegistryRepository(database: await _database());
  }

  Future<void> _loadRegistrySnapshot() async {
    try {
      ...
      setState(() {
        _registryLoading = false;
        _registryStatusMessage = '公司行號資料狀態讀取失敗：$error';
      });
    }
  }

  Future<void> _updateBusinessRegistry(BuildContext context) async {
    if (_registryUpdating) return;
    final messenger = ScaffoldMessenger.of(context);
    setState(() {
      _registryUpdating = true;
      _registryStatusMessage = '正在下載並驗證公司行號資料…';
    });
    try {
      final db = await _database();
      final service = widget.registryUpdateService ??
          BusinessRegistryUpdateService(database: db is Database ? db : null);
      final result = await service.update();
      if (!mounted) return;
      setState(() {
        _registrySnapshot = result.snapshot;
        _registryStatusMessage = switch (result.status) {
          BusinessRegistryUpdateStatus.updated =>
            '公司行號資料已完成驗證並切換至 ${result.snapshot?.version ?? '新版'}。',
          BusinessRegistryUpdateStatus.alreadyCurrent =>
            '目前公司行號資料已是最新版本 ${result.snapshot?.version ?? ''}。',
          BusinessRegistryUpdateStatus.distributionNotConfigured =>
            '此建置尚未設定公司行號資料發布端點；保留目前本機資料。',
        };
      });
      ...
      if (mounted) setState(() => _registryUpdating = false);
    }
  }
  ...
  Future<void> _loadNotificationSettings() async {
    try {
      ...
      if (!mounted) return;
      setState(
        () => _notificationSettings = BackupNotificationSettings.defaults(),
      );
    }
  }
  ...
  Future<void> _handleReminderIntervalDaysChanged(int intervalDays) async {
    await _saveReminderSettings(
      _reminderSettings.copyWith(intervalDays: intervalDays),
    );
  }
  ...
  Future<void> _handleAutomaticBackupEnabledChanged(bool enabled) async {
    await _saveReminderSettings(
      _reminderSettings.copyWith(
        ...
        networkUsageAllowed:
            enabled ? _reminderSettings.networkUsageAllowed : false,
        cloudBackupHandoffEnabled:
            enabled ? _reminderSettings.cloudBackupHandoffEnabled : false,
      ),
      successMessage:
          enabled ? '自動建立備份已開啟；可另外開啟雲端備份交接' : '自動建立備份已關閉',
    );
  }
  ...
  Future<void> _handleNetworkUsageAllowedChanged(bool allowed) async {
    await _saveReminderSettings(
      _reminderSettings.copyWith(
        ...
        cloudBackupHandoffEnabled:
            allowed ? _reminderSettings.cloudBackupHandoffEnabled : false,
      ),
      successMessage: allowed
          ? '已允許備份流程使用網路流量；請留意行動數據用量'
          : '已關閉備份流程網路流量使用，雲端備份交接也已關閉',
    );
  }

  Future<void> _handleCloudBackupHandoffEnabledChanged(bool enabled) async {
    final allowed = _reminderSettings.automaticBackupEnabled &&
        _reminderSettings.networkUsageAllowed;
    if (enabled && !allowed) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('請先開啟自動建立備份並允許備份使用網路流量。')),
      );
      return;
    }
    await _saveReminderSettings(
      ...
      successMessage: enabled
          ? '自動雲端備份交接已開啟；到期備份後會開啟系統分享面板'
          : '自動雲端備份交接已關閉',
    );
  }

  Future<void> _saveReminderSettings(
    BackupReminderSettings settings, {
    String successMessage = '備份提示設定已儲存，提醒排程已同步',
  }) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('備份提示設定儲存失敗：$error')),
      );
    }
  }
  ...
  Future<void> _maybeRunAutomaticBackup() async {
    ...
    if (settings.lastBackupAt == null ||
        !settings.isReminderDue(DateTime.now().toUtc())) {
      return;
    }
    ...
    try {
      final db = await _database();
      final file = await widget.actionService.fullBackupService
          .writeFullBackupFile(db, sourcePlatform: 'android');
      ...
      await widget.reminderSettingsRepository.recordBackupCompleted(
        db,
        completedAt,
      );
      final updatedSettings =
          await widget.reminderSettingsRepository.load(db);
      if (mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('自動建立備份失敗：$error')),
        );
      }
    } finally {
  ...
  Future<void> _handleNotificationPermissionStatusChanged(
    BackupNotificationPermissionStatus status,
  ) async {
    await _saveNotificationSettings(
      _notificationSettings.copyWith(
  ...
  Future<void> _requestNotificationPermission(BuildContext context) async {
    ...
    try {
      ...
      await _saveNotificationSettings(
        next,
        successMessage: '系統通知權限狀態已更新：${status.label}；提醒排程已同步',
      );
    } catch (error) {
      if (!context.mounted) return;
      messenger.showSnackBar(
        SnackBar(content: Text('要求通知權限失敗：$error')),
      );
    }
  }
  ...
  Future<void> _saveNotificationSettings(
    ...
    String successMessage = '備份通知 readiness 已儲存，提醒排程已同步',
  }) async {
    setState(() => _notificationSettings = settings);
    try {
      final db = await _database();
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('備份通知 readiness 儲存失敗：$error')),
      );
    }
  }
  ...
  Future<void> _sendBackupReminderNotification(BuildContext context) async {
    final messenger = ScaffoldMessenger.of(context);
    try {
      final result = await LocalBackupReminderNotificationService(
        port: widget.notificationPort,
      ).sendSmokeReminder(
        notificationSettings: _notificationSettings,
        now: DateTime.now().toUtc(),
      );
      ...
      if (result.sent) {
        messenger.showSnackBar(
          SnackBar(
            content: Text('已送出備份提醒 smoke：${result.notification?.body ?? ''}'),
          ),
        );
        return;
      }
      messenger.showSnackBar(
        SnackBar(
          content: Text('未送出備份提醒：${_notificationSkipReasonLabel(result.reason)}'),
        ),
      );
    } catch (error) {
      if (!context.mounted) return;
      messenger.showSnackBar(
        SnackBar(content: Text('備份提醒 smoke 失敗：$error')),
      );
    }
  }
  ...
  Future<void> _runFullBackup(BuildContext context) async {
    ...
    try {
      ...
      await _reconcileSchedule();
      messenger.showSnackBar(
        SnackBar(content: Text('${result.message}；已更新備份提示時間與提醒排程')),
      );
    } catch (error) {
      messenger.showSnackBar(
        SnackBar(content: Text('檔案操作失敗：$error')),
      );
    }
  }
  ...
  Future<void> _openFullRestoreFlow(BuildContext context) async {
    ...
    if (grant != null) {
      await _showSafeSource(
        context,
        () => widget.actionService.prepareFullRestoreSourceFromGrant(
          grant,
          currentDb: db,
        ),
        currentDb: db,
    ...
    await _showSafeSource(
      context,
      () => widget.actionService.prepareFullRestoreSource(currentDb: db),
      currentDb: db,
    );
  }
  ...
  Future<void> _pickFullRestoreSource(BuildContext context) async {
    ...
    try {
      ...
      if (result == null) {
        messenger.showSnackBar(
          const SnackBar(content: Text('未選擇完整備份來源')),
        );
        return;
      }
      final db = await _database();
      ...
      messenger.showSnackBar(
        SnackBar(content: Text('已授權還原來源：${result.grant.displayName}')),
      );
      await _showSafeSource(
        context,
        () => widget.actionService.prepareFullRestoreSourceFromPickResult(
          result,
          currentDb: db,
        ),
        currentDb: db,
      ...
      if (!mounted) return;
      messenger.showSnackBar(
        SnackBar(content: Text('選擇完整還原來源失敗：$error')),
      );
    }
  }
  ...
  Future<void> _pickReadableImportFile(BuildContext context) async {
    ...
    try {
      ...
      if (path == null) {
        messenger.showSnackBar(
          const SnackBar(content: Text('未選擇 readable 匯入檔')),
        );
        return;
      }
      final db = await _database();
      ...
      await _showSafeSource(
        context,
        () => widget.actionService.prepareReadableImportSourceFromPath(
          path,
          currentDb: db,
        ),
        currentDb: db,
      ...
      if (!mounted) return;
      messenger.showSnackBar(
        SnackBar(content: Text('選擇 readable 匯入檔失敗：$error')),
      );
    }
  }
  ...
  Future<void> _runFileAction(
    ...
    Future<BackupMigrationActionResult> Function() action,
  ) async {
    final messenger = ScaffoldMessenger.of(context);
    messenger.showSnackBar(const SnackBar(content: Text('正在準備檔案...')));
    try {
  ...
  Future<void> _showSafeSource(
    BuildContext context,
    Future<SafeImportSourceResult> Function() prepareSource, {
    ...
    try {
      ...
      await showDialog<void>(
        ...
        builder: (dialogContext) {
          ...
          final hasReadableCandidates =
              result.readableImportCandidates.isNotEmpty;
          return AlertDialog(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (result.restoreSourceGrant != null) ...[
                    ...
                    Text(
                      '授權時間：${result.restoreSourceGrant!.grantedAt.toLocal()}',
                    ),
                  ],
                  const SizedBox(height: 12),
                  ...
                  if (hasRestorePreview)
                    const Text(
                      '安全狀態：正式還原前仍需輸入 RESTORE，且會先建立還原前備份。',
                    )
                  else if (hasReadableCandidates)
                    const Text(
                      '安全狀態：readable 匯入仍需 dry-run、mapping review 與確認後才會寫入交易列。',
                    ),
                  const SizedBox(height: 12),
                  _RestorePreviewSection(
                    ...
                    onCommitRestore: currentDb is Database
                        ? (preview) async {
                            ...
                            await _confirmAndCommitFullRestore(
                              context,
                              currentDb,
                              preview,
                            );
                  ...
                  if (result.readableImportCandidates.isNotEmpty) ...[
                    ...
                    _ReadableImportCandidateSection(
                      ...
                      onOpenReviewFlow: currentDb == null
                          ...
                          : (candidate) async {
                              Navigator.of(dialogContext).pop();
                              await _openReadableImportReviewFlow(
                                context,
                                currentDb,
                                candidate,
                              );
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('準備安全來源失敗：$error')),
      );
    }
  }

  Future<void> _confirmAndCommitFullRestore(
    BuildContext context,
    Database db,
    FullRestoreBackupPreview preview,
  ) async {
    ...
    try {
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const Text(
                  '此操作會以備份檔覆蓋目前資料。系統會先建立還原前備份，若還原或驗證失敗會 rollback。',
                ),
                const SizedBox(height: 12),
      ...
      if (confirmation.trim() !=
          FullRestoreService.destructiveConfirmationText) {
        if (!context.mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('未輸入 RESTORE，完整還原已阻擋。')),
        );
        return;
      }
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('正在執行完整還原...')),
      );
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('完整還原失敗，已保留原資料：$error')),
      );
    } finally {
      controller.dispose();
    }
  ...
  Future<void> _openReadableImportReviewFlow(
    BuildContext context,
    DatabaseExecutor db,
    ReadableImportSourceCandidate candidate,
  ) async {
    final dryRun = candidate.dryRunResult;
    if (dryRun == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('此候選檔尚未完成 dry-run preview，無法開啟審核流程。')),
      );
      return;
    }
    try {
      final analysis =
          await const ImportMappingAnalysisService().analyze(db, dryRun);
      if (!context.mounted) return;
      await showDialog<void>(
        context: context,
                ...
                Padding(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Text(
                          'readable 匯入審核｜${candidate.fileName}',
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('開啟匯入審核流程失敗：$error')),
      );
    }
  }
}

String _notificationSkipReasonLabel(
  LocalBackupReminderNotificationSkipReason reason,
) {
  switch (reason) {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        const Text(
          '此區預覽完整備份檔 metadata 與 table diff；完成確認後可進入 RESTORE typed confirmation。',
        ),
        const SizedBox(height: 8),
        for (final preview in previews)
          _RestorePreviewCard(
            preview: preview,
            onCommitRestore: onCommitRestore,
          ),
      ],
    );
  }
        ...
        child: Column(
          ...
          children: [
            ...
            if (metadata != null) ...[
              ...
              Text(
                'schema_version：${metadata.databaseSchemaVersion ?? '-'}',
              ),
              Text('created_at：${metadata.createdAt}'),
              Text('source_platform：${metadata.sourcePlatform}'),
              const SizedBox(height: 8),
              Text(
                'tables：${preview.tableRowCounts.entries.map((entry) => '${entry.key}=${entry.value}').join(', ')}',
              ),
              const SizedBox(height: 8),
              _TableImpactSummary(impacts: preview.tableImpacts),
              BackupPreviewConfirmationGate(
                preview: preview,
                onCommitRestore: onCommitRestore,
              ),
            ],
...
class _TableImpactSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (impacts.isEmpty) {
      return const Text('table diff：尚無目前資料庫筆數可比較。');
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        for (final impact in impacts)
          Text(
            '${impact.tableName}：目前 ${impact.currentCount} / 備份 ${impact.backupCount} / 差異 ${impact.delta} / ${impact.level.name}',
          ),
      ],
    );
  }
...
class _ReadableImportCandidateSection extends StatelessWidget {
  const _ReadableImportCandidateSection({
    required this.candidates,
    this.onOpenReviewFlow,
  });
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const Text(
          'readable 匯入候選｜交易 CSV / JSON',
          style: TextStyle(fontWeight: FontWeight.w800),
        ),
        const SizedBox(height: 4),
        const Text(
          '此區只處理交易列，會經過 dry-run、mapping、review、confirmation 與 commit；不是完整還原。',
        ),
        const SizedBox(height: 8),
        for (final candidate in candidates)
          _ReadableImportCandidateCard(
            candidate: candidate,
            onOpenReviewFlow: onOpenReviewFlow,
          ),
      ],
    );
  }
...
class _ReadableImportCandidateCard extends StatelessWidget {
  const _ReadableImportCandidateCard({
  ...
  Widget build(BuildContext context) {
    final dryRun = candidate.dryRunResult;
    final canOpenReview =
        candidate.isValid && dryRun != null && onOpenReviewFlow != null;
    return Card(
      margin: const EdgeInsets.only(bottom: 8),
      child: Padding(
        padding: const EdgeInsets.all(12),
        child: Column(
          ...
          children: [
            ...
            if (dryRun != null) ...[
              const SizedBox(height: 8),
              Text(
                'dry-run rows：total ${dryRun.totalRows} / valid ${dryRun.validRows} / invalid ${dryRun.invalidRows} / duplicate ${dryRun.duplicateRows} / ready ${dryRun.readyToInsertRows}',
              ),
            ...
            Align(
              ...
              child: FilledButton.icon(
                onPressed: canOpenReview
                    ? () => onOpenReviewFlow?.call(candidate)
                    : null,
