...
class BusinessRegistryNationwideBuildPass {
  ...
  static BusinessRegistryEntity normalizeEntity(
    ...
    return BusinessRegistryEntity(
      ...
      officialFields: BusinessRegistryEntity.normalizeOfficialFields(
        source.officialFields,
      ),
    );
  }
  ...
  final String registryContentSha256;
    ...
    if (entity.officialFields.isNotEmpty) {
      ...
      if (unknown.isNotEmpty) {
        errors.add('REGISTRY_BUILDER_OFFICIAL_DETAIL_FIELD_NOT_ALLOWED');
      }
      ...
      if (officialSeller.isNotEmpty && officialSeller != entity.sellerIdentifier) {
        errors.add('REGISTRY_BUILDER_OFFICIAL_DETAIL_SELLER_MISMATCH');
      }
      final officialName = entity.officialFields['營業人名稱']?.trim() ?? '';
      if (officialName.isNotEmpty && officialName != entity.legalName.trim()) {
        errors.add('REGISTRY_BUILDER_OFFICIAL_DETAIL_LEGAL_NAME_MISMATCH');
      }
    }
    return List<String>.unmodifiable(errors);
  }
