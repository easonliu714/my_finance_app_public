...
def validate_gcis_closure(
    ...
    *,
    ...
    for field in (
        ...
        "seller_set_sha256",
        "globally_sorted_combined_evidence_sha256",
    ):
    ...
    artifact_sha = artifact_sha256.lower()
    if not SHA256_RE.fullmatch(artifact_sha):
        raise RuntimeError("GCIS_ARTIFACT_SHA_INVALID")
    closure_sha = hashlib.sha256(_canonical_bytes(closure)).hexdigest()
    return closure_sha
...
def build_authority_bundle(
    *,
    ...
    closure: dict[str, object],
    gcis_workflow_run_id: int,
    gcis_artifact_id: int,
    gcis_artifact_sha256: str,
) -> dict[str, object]:
    ...
    closure_sha = validate_gcis_closure(
        closure,
        head,
        workflow_run_id=gcis_workflow_run_id,
        artifact_id=gcis_artifact_id,
        artifact_sha256=gcis_artifact_sha256,
    )
    ...
    bundle: dict[str, object] = {
        "schema_version": 1,
        "gate": "P4.20.3-E-authority-bundle",
        "exact_head": head,
        ...
        "gcis_workflow_run_id": gcis_workflow_run_id,
        "gcis_closure_sha256": closure_sha,
        "gcis_artifact_id": gcis_artifact_id,
        "gcis_artifact_sha256": gcis_artifact_sha256.lower(),
        "gcis_seller_count": closure["seller_count"],
        "gcis_success_count": closure["total_success_count"],
        "gcis_failure_count": closure["total_failure_count"],
...
def self_test() -> None:
    ...
    assert bundle["gcis_zero_silent_drop"] is True
    old = dict(closure); old["exact_head"] = "2"*40
    try:
        build_authority_bundle(exact_head=head, source=source, closure=old, gcis_workflow_run_id=15, gcis_artifact_id=1, gcis_artifact_sha256="9"*64)
        raise AssertionError("EXPECTED_OLD_HEAD_HOLD")
    except RuntimeError as exc:
        assert str(exc) == "GCIS_CLOSURE_EXACT_HEAD_MISMATCH"
    bad = dict(closure); bad["total_success_count"] -= 1
    try:
        build_authority_bundle(exact_head=head, source=source, closure=bad, gcis_workflow_run_id=15, gcis_artifact_id=1, gcis_artifact_sha256="9"*64)
        raise AssertionError("EXPECTED_SILENT_DROP_HOLD")
    except RuntimeError as exc:
        assert str(exc) == "GCIS_CLOSURE_SILENT_DROP_DETECTED"
    ...
    try:
        build_authority_bundle(exact_head=head, source=missing, closure=closure, gcis_workflow_run_id=15, gcis_artifact_id=1, gcis_artifact_sha256="9"*64)
        raise AssertionError("EXPECTED_REPLAY_FIELD_HOLD")
    except RuntimeError as exc:
        assert str(exc) == "SOURCE_AUTHORITY_SHA_FIELD_INVALID:source_csv_sha256"
