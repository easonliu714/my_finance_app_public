...
void main() {
  ...
  test('vendored flutter_pdf_text uses temp-file-backed PDFBox policy', () {
    ...
    expect(
      RegExp(r'^version:\s*0\.9\.0\s*$', multiLine: true).hasMatch(packagePubspec),
      isTrue,
    );
    expect(license, contains('MIT License'));
    expect(license, contains('Copyright (c) 2020 Alessio Luciani'));
    expect(pluginSource, contains('import com.tom_roush.pdfbox.io.MemoryUsageSetting'));
    expect(pluginSource, contains('private lateinit var applicationContext: Context'));
    ...
    expect(pluginSource, contains('.setTempDir(applicationContext.cacheDir)'));
    expect(pluginSource, contains('PDDocument.load(File(path), password, memoryUsageSetting)'));
    expect(
      RegExp(r'PDDocument\.load\(\s*File\(path\)\s*,\s*password\s*\)').hasMatch(pluginSource),
      isFalse,
    );
    ...
    expect(
      androidBuild,
      contains("implementation 'io.legere:pdfiumandroid:1.0.35'"),
    );
    expect(androidBuild, contains('minSdkVersion 23'));
    expect(candidateScope, contains('class PdfiumCloudAwardSortedPdfCandidateLookup'));
    ...
    expect(appExtractor, contains('flutter_pdf_text-0.9.0-android-chunk8-diagnostic-v3'));
